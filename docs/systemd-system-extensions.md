...
System extension images (or sysext images) can be disk image files or simple folders that get loaded by the 
`systemd-sysext.service`. They provide a way to dynamically extend the `/usr` and `/opt` directory hierarchies with 
additional files. When one or more system extension images are activated, their `/usr` and `/opt` hierarchies are 
combined via "overlayfs" with the same hierarchies of the host OS. This causes "merging" (or overmounting) of the 
`/usr` and `/opt` contents of the system extension image with that of underlying host system.
...
a package manager like `zypper` which can be used to install additional packages. System extension helps extend the 
functionality and usability of such minimal OS.
...
## Create system extension images

There are multiple tools to create a system extension image, e.g., `mksquashfs`, `mkerofs`, `mkosi`. In this guide we 
will use `mkosi` which is a higher level tool than the other two. It helps build bootable OS images, system 
...
### Embed a binary in system extension image
...
1. Prepare the `mkosi.extra` directory inside the `example-extension`:
   - Create the directory structure for elemental3ctl:
        ```shell
        mkdir -p example-extension/mkosi.extra/usr/bin
        ```       
   - Get the `kubectl` binary by following steps mentioned
...
└── mkosi.images
    ...
    └── tools
        ...
        └── mkosi.finalize
```
The presence of `mkosi.images` directory indicates that the configuration is meant for multiple images.
...
- `tools/mkosi.conf`: This is the configuration for tools layer.
...
[building linux image](./building-linux-image.md#configuring-through-a-configuration-script).